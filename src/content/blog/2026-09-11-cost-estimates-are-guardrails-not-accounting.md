...
The second is worse. Our [security page](/security) says telemetry is "off by default." That is true of the failure-fingerprint sharing, which is gated behind a config flag and genuinely opt-in. It is not true of the run record this post is built on: `reportRun` is called unconditionally at the end of every `satus generate`, with no environment variable or config key to turn it off. The payload is minimal and carries no schema, table names, or row data, and v0.3.7 cut it back to keep that promise. But "off by default" describes something the CLI does not do, and a privacy claim that is only mostly true is not one we want on that page. We are fixing the gap rather than the sentence.

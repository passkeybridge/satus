...
A useful starting reference for how skewed real appointment behaviour is comes from the published literature on outpatient no-shows. Reported rates vary widely by specialty and country, but a frequently cited systematic review of outpatient no-show research puts the range broadly between roughly 10% and 30% across settings ([Dantas et al., 2018, "No-shows in appointment scheduling – a systematic literature review", *Health Policy*](https://www.sciencedirect.com/science/article/pii/S0168851018300484)). The exact rate is not the point. The point is that any fixture that treats `status = 'no_show'` as a 1%-of-rows curiosity will hide problems in code paths that, in production, fire constantly.

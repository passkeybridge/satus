...
Our pricing FAQ says "Free is fully offline forever." In the context of that answer, which is about license verification, it is true: Free never contacts the license server, because there is no key to verify. Read plainly, it is not, and the plain reading is the one a user will take.

The reason is the same one covered in [the cost post](/blog/cost-estimates-are-guardrails-not-accounting): `reportRun` is called unconditionally at the end of every `satus generate`, on every tier. The payload is deliberately small and carries a run UUID, the CLI version, provider, model, profile, a table *count*, row and token totals, an estimated spend, a duration, and on failure a fixed-vocabulary error class. No schema name, no table names, no column names, no row data. v0.3.7 cut it back to exactly that, and the ingest endpoint discards the old fields from any older client that still sends them.

That payload is defensible. Describing it as "off by default" on our [security page](/security), and the tier that sends it as "fully offline", is not. Only the separate failure-fingerprint sharing is genuinely opt-in, behind a `telemetry.share_failure_fingerprints` config key. We are closing the gap in the CLI rather than softening the sentences on the site, which is the same call we made when the [telemetry payload itself](/blog/v0-3-7-release-notes) contradicted the privacy page in July.

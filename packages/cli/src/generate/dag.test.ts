...
describe("topoSort", () => {
  ...
  it("reports unresolved cycle when no back-edge is nullable", () => {
    const tables = [
      tbl("a", {
        cols: [{ name: "id" }, { name: "b_id" }],
        fks: [{ column: "b_id", refTable: "b" }],
      }),
      tbl("b", {
        cols: [{ name: "id" }, { name: "a_id" }],
        fks: [{ column: "a_id", refTable: "a" }],
      }),
    ];
    const r = topoSort(tables);
  ...
  it("treats FKs to tables outside the target set as satisfied", () => {
    const tables = [
      tbl("posts", {
        cols: [{ name: "id" }, { name: "tenant_id" }],
        fks: [{ column: "tenant_id", refTable: "tenants_outside" }],
      }),
    ];
    const r = topoSort(tables);
    expect(r.cycle).toBeNull();

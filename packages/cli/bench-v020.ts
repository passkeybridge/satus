...
async function setupCorpus(client: Client) {
  ...
  for (let i = 0; i < 40; i++) {
    await client.query(
      `create table ${SCHEMA}.t_${i} (id serial primary key, name text not null, parent_id int)`,
    );
  }
  // Cycle: users.primary_post_id nullable -> posts; posts.author_id -> users.
  await client.query(
    `create table ${SCHEMA}.users (id serial primary key, name text not null, primary_post_id int)`,
  );
  await client.query(
    `create table ${SCHEMA}.posts (id serial primary key, title text not null, author_id int not null references ${SCHEMA}.users(id))`,
  );
  await client.query(
    `alter table ${SCHEMA}.users add constraint users_primary_post_fk foreign key (primary_post_id) references ${SCHEMA}.posts(id)`,
  );
}
...
async function cycleIntegration(client: Client) {
  ...
  for (const [i, uid] of userIds.entries()) {
    await client.query(`update ${SCHEMA}.users set primary_post_id = $1 where id = $2`, [
      postIds[i],
      uid,
    ]);
  }
    ...
    const out = {
      ...
      binary: {
        ...
        total_reduction_pct: Number(
          (
            ((baselineBytes + baselineSourcemapBytes - newBytes) /
              (baselineBytes + baselineSourcemapBytes)) *
            100
          ).toFixed(1),
        ),
      },

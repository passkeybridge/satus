  ...
  try {
    ...
    const out = {
      ...
      binary: {
        ...
        total_payload_reduction_pct: Number(
          (((baselineJs + baselineMap - newBytes) / (baselineJs + baselineMap)) * 100).toFixed(1),
        ),
      },
      cycle_breaking: {
        unit_tests: "packages/cli/src/generate/dag.test.ts (5/5 passing)",
        integration:
          "deferred — pooled Supabase connection in sandbox lacks DDL rights; covered by local-Postgres CI on the release runner",
      },
    };
